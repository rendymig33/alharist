...
class Transaksi_model extends Model
{
        ...
        return "
            (
                {$itemAlias}.line_total - (
                    CASE
                        WHEN COALESCE({$masterAlias}.category, '') = 'E-SALDO' THEN {$itemAlias}.qty * COALESCE({$itemAlias}.purchase_price, 0)
                        ELSE {$itemAlias}.qty * (
                            CASE
    ...
    public function createSale(array $payload): void
    {
        ...
        $saleStatement->execute([
            ...
            'shift' => max(1, (int) ($payload['shift'] ?? 1)),
        ]);
    ...
    public function salesList(string $keyword = ''): array
    {
        ...
        $sql = "
            SELECT sales.*,
                   ...
                   CASE
                       WHEN COUNT(sale_items.id) > 0
                            AND SUM(CASE WHEN COALESCE(items.category, '') = 'E-SALDO' THEN 1 ELSE 0 END) = COUNT(sale_items.id)
                       THEN 'E-Transaction'
    ...
    {
        $statement = $this->db->prepare("
            SELECT sale_items.*, items.name AS item_name, items.code AS item_code, COALESCE(items.category, '') AS item_category,
    ...
    public function deleteSale(int $saleId): bool
    {
        ...
        foreach ($saleItems as $item) {
            $itemMaster = $this->db->prepare("SELECT COALESCE(category, '') FROM items WHERE id = :id LIMIT 1");
            $itemMaster->execute(['id' => $item['item_id']]);
            $itemCategory = (string) $itemMaster->fetchColumn();
            if ($itemCategory !== 'E-SALDO') {
